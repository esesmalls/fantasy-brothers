...
最新：**装配台编写增量** 可调底座锚点与盘面裁取（默认等于现行范围）、Ctrl 多选统一变换，以及剑／矛／弓关键帧编写。双击 `Paperdoll.cmd`。[编写说明](docs/28-paperdoll-authoring.md) · [装配台 v2](docs/27-modular-paperdoll.md)。未改的草案保持原显示；仍为单体型试样，不视为全人物库已验收。
