...
func _ready() -> void:
	...
	view_choice=_choice(toolbar,["装配与对照","穿戴组合检查","战场尺寸检查"],["fit","matrix","field"],func(v):view=v;playing=false;refresh(),"fit")
...
func refresh() -> void:
	...
	view_choice.select(["fit","matrix","field"].find(view))
	...
	status.text=("● 未保存  " if document.dirty() else "已保存 / v3 基准  ")+_notice
	stage.queue_redraw()
...
func draw_stage(c:Control) -> void:
	...
	Actor.draw_actor(c,at,s,armor,weapon,damaged,wounded,progress,"hit",catalog_data,hidden_layers)
	...
	if ghost:Actor.draw_actor(c,at,s,armor,weapon,damaged,wounded,progress,"hit",document.baseline,[],Color(.6,.85,1,ghost_alpha))
	...
	Actor.draw_actor(c,Vector2(64,small_y),1,armor,weapon,damaged,wounded,0,"hit",catalog_data)
	Actor.draw_actor(c,Vector2(215,small_y),2,armor,weapon,damaged,wounded,0,"hit",catalog_data)
	Actor.draw_actor(c,Vector2(c.size.x-130,small_y),2,armor,weapon,damaged,wounded,0,"hit",document.baseline)
...
func _draw_matrix(c:Control) -> void:
	...
	for row in range(3):
		for col in range(3):
			...
			Actor.draw_actor(c,at,s,a,weapon,row>0,row==2,0,"hit",catalog_data)
...
func _draw_field(c:Control) -> void:
	...
	for row in range(2):
		...
		for i in range(3):
			...
			Actor.draw_actor(c,at,s,["linen","padded","mail"][i],["sword","spear","bow"][i],damaged,wounded,0,"hit",catalog_data)
...
func open_project(path:String) -> bool:
	...
	if error.is_empty():project_path=path;_notice="已载入："+path
	else:_notice=error
...
func capture_all(output:String) -> void:
	...
	for item in [["fit","green"],["matrix","green"],["field","earth"],["fit","light"]]:
