...
static func duration(weapon: String) -> float:
	return {"sword":0.86, "spear":0.82, "bow":1.08}.get(weapon, 0.86)

static func contact(weapon: String) -> float:
	return 0.70 if weapon == "bow" else 0.42

static func release(weapon: String) -> float:
	return 0.42 if weapon == "bow" else 0.25

static func sample(weapon: String, progress: float, outcome: String = "hit") -> Dictionary:
	var p := clampf(progress, 0, 1)
	var frames: Array
	if weapon == "spear":
		frames = [[0.0,23.0,-26.0,0.52],[0.25,17.0,-25.0,0.58],
			[0.42,35.0,-31.0,PI/2],[0.53,35.0,-31.0,PI/2],
			[0.64,31.0,-31.0,PI/2],[1.0,23.0,-26.0,0.52]]
	elif weapon == "bow":
		frames = [[0.0,48.0,-38.0,0.0],[0.42,45.0,-38.0,-0.055],
			[0.49,50.0,-38.0,0.035],[0.60,47.0,-38.0,-0.015],
			[1.0,48.0,-38.0,0.0]]
	else:
