...
func run() -> void:
	...
	ui.select_group("head");ui.weapon="none";ui.refresh()
	var point:Vector2=ui.pointer_for("head")
	var button:=InputEventMouseButton.new();button.button_index=MOUSE_BUTTON_LEFT;button.pressed=true;button.position=point;ui.stage_input(button)
	var motion:=InputEventMouseMotion.new();motion.position=point+Vector2(ui.camera().scale*2,0);ui.stage_input(motion)
